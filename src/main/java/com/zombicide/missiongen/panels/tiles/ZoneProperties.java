...
public class ZoneProperties extends ZonePropertiesPanel implements BoardSelectionListener {
    ...
    private Map<BoardGameAsset, JCheckBox> assetCheckboxes;
    ...
    private void initComponents() {
        ...
        // Initialize asset checkboxes
        assetCheckboxes = new HashMap<>();
        for (BoardGameAsset asset : BoardGameAsset.values()) {
            JCheckBox checkbox = new JCheckBox(asset.toString());
            checkbox.setEnabled(false);
            checkbox.addActionListener(e -> onAssetCheckboxChanged(asset, checkbox.isSelected()));
    ...
    private void setupLayout() {
        ...
        for (BoardGameAsset asset : BoardGameAsset.values()) {
            ...
            add(assetCheckboxes.get(asset), gbc);
        }
    ...
    private void updateAssetCheckboxes() {
        boolean hasSelection = !this.selectedAreas.isEmpty();

        // Enable/disable all asset checkboxes based on selection
        for (JCheckBox checkbox : assetCheckboxes.values()) {
            checkbox.setEnabled(hasSelection);
        }

        if (!hasSelection) {
            // Clear all checkboxes if no areas selected
            for (JCheckBox checkbox : assetCheckboxes.values()) {
        ...
        // Get assets from the first selected area
        BoardArea firstArea = this.selectedAreas.get(0);
        java.util.List<BoardGameAsset> currentAssets = firstArea.getBoardGameAssets();

        if (currentAssets == null) {
            currentAssets = new java.util.ArrayList<>();
        }

        // Update checkbox states based on current assets
        for (Map.Entry<BoardGameAsset, JCheckBox> entry : assetCheckboxes.entrySet()) {
            BoardGameAsset asset = entry.getKey();
            JCheckBox checkbox = entry.getValue();
            checkbox.setSelected(currentAssets.contains(asset));
        }
    }

    private void onAssetCheckboxChanged(BoardGameAsset asset, boolean isSelected) {
        if (selectedAreas == null || selectedAreas.isEmpty()) {
            return;
        }

        for (BoardArea area : selectedAreas) {
            if (area.getBoardGameAssets() == null) {
                area.setBoardGameAssets(new java.util.ArrayList<>());
            }

            if (isSelected) {
                area.addBoardGameAsset(asset);
                logger.info("Added {} to Area {}", asset, area.getAreaId());
            } else {
                logger.info("Removed {} from Area {}", asset, area.getAreaId());
            }
        }
        notifyAreasChanged();
    }
    ...
    private void onSplitHorizontal() {
        ...
        if (zoneTileDraw != null) {
            zoneTileDraw.enterSplitMode(
                    com.zombicide.missiongen.panels.tiles.ZoneTileDraw.DrawMode.SPLIT_HORIZONTAL,
                    areaToSplit);
        }
    }
    ...
    private void onSplitVertical() {
        ...
        if (zoneTileDraw != null) {
            zoneTileDraw.enterSplitMode(
                    com.zombicide.missiongen.panels.tiles.ZoneTileDraw.DrawMode.SPLIT_VERTICAL,
                    areaToSplit);
        }
    }
}
